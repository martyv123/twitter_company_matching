...
def author_company_match():
    """
    Match authors with their respective companies.
    Writes to a temporary file called `authors_2_company_tmp.csv'.
    """
    global AUTHOR_COMPANIES
    ...
    with open('authors_2_company.csv', mode='r', encoding='utf-8-sig') as csv_file:
        csv_reader = csv.DictReader(csv_file)
        count = 0
        for row in csv_reader:
            if count == 50:
                break
            count += 1
            print("Author Company Match: " + str(count))
            # fuzzywuzzy matching for best match
            author = row["Author"]
            match_tuple = process.extractOne(author, company_handles)
            match = match_tuple[0]
            ...
            if match_score >= 60:
                ...
                match_dict = {"Author": author, "cik": '', "conm": '', "match_score": '', 
                                "TwitterHandle": '', "TwitterHandle2": ''}
                AUTHOR_COMPANIES.append(match_dict)

    # Write to tmp file authors_2_company_tmp.csv
    with open('authors_2_company_tmp.csv', mode='w', encoding='utf-8-sig', newline='') as csv_file:
        fieldnames = ["Author", "cik", "conm", "match_score", "TwitterHandle", "TwitterHandle2"]
        csv_writer = csv.DictWriter(csv_file, fieldnames=fieldnames, delimiter=',')
        csv_writer.writeheader()
        csv_writer.writerows(AUTHOR_COMPANIES)
...
if __name__ == "__main__":
    ...
    print("Matching authors with their respective companies...")
    author_company_match()
